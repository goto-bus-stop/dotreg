//! Parser functions for registry files.
//!
//! Many of these are argument-less functions because previously, they took `version` arguments
//! for Wine registry file support. Whether Wine hives should be supported by this crate is
//! currently unclear but I kept the function structure in place in case I want to add support for
//! Wine hives back in the future.
use crate::{
...
use nom::{
    ...
    character::complete::{crlf, hex_digit1},
...
fn reg_key_header() -> impl Fn(&str) -> IResult<&str, &str> {
    move |input: &str| {
...
fn reg_value() -> impl Fn(&str) -> IResult<&str, RegValue> {
    move |input: &str| {
        let dword = preceded(
            tag("dword:"),
...
fn reg_value_line() -> impl Fn(&str) -> IResult<&str, (String, RegValue)> {
    move |input: &str| {
        let (input, tuple) = separated_pair(reg_value_name, tag("="), reg_value())(input)?;
        let (input, _) = crlf(input)?;
...
fn reg_values() -> impl Fn(&str) -> IResult<&str, Vec<(String, RegValue)>> {
    move |input: &str| {
        let (input, values) = many0(reg_value_line())(input)?;
        Ok((input, values))
    }
}

fn reg_key() -> impl Fn(&str) -> IResult<&str, RegKey> {
    move |input: &str| {
        let (input, name) = reg_key_header()(input)?;
        let (input, values) = reg_values()(input)?;
        let (input, _) = crlf(input)?;
...
pub fn reg_file(input: &str) -> IResult<&str, RegFile> {
    ...
    let (input, keys) = many0(reg_key())(input)?;
    for key in keys {
...
mod tests {
    ...
    fn reg_key_header_test() {
        let (_, res) = reg_key_header()("[HKEY_CURRENT_USER\\Software\\Microsoft\\Microsoft Games\\Age of Empires II: The Conquerors Expansion]\r\n").unwrap();
    ...
    fn reg_value_line_test() {
        ...
        let (_, res) = reg_value_line()("@=dword:00000000\r\n").unwrap();
        assert_eq!(res, ("@".to_string(), RegValue::Dword(0)));
    }

    #[test]
    fn reg_key_test() {
        let (_, key) = reg_key()("[HKEY_CURRENT_USER\\Software\\Microsoft\\Microsoft Games\\Age of Empires II: The Conquerors Expansion\\1.0\\EULA]\r\n\"FIRSTRUN\"=dword:00000001\r\n\r\n").unwrap();
